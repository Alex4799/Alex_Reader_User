<template>
    <div>
        ...
        <table class="table table-striped rounded">
            ...
            <tbody>
                <tr class="text-center" v-for="(item,index) in categories" :key="index">
    ...
    export default {
        ...
        methods: {
            ...
            searchCategory(){
                if (this.searchKey!='') {
                    this.loadingStatus=true;
                    axios.get(`http://alexmedia.alexlucifer.info/api/user/category/search/${this.searchKey}`,{headers:this.header}).then((response)=>{
                        this.categories=response.data;
                        this.loadingStatus=false;
                        this.categoryCount=this.categories.length;
