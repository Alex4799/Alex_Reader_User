    ...
    export default {
        ...
        methods: {
            getCategory (page=1) {
                ...
                axios.get(`http://alexmedia.alexlucifer.info/api/user/category/main/get?page=${page}`,{headers:this.header}).then((response)=>{
            ...
            deleteCategory(id){
                this.loadingStatus=true;
                axios.get(`http://alexmedia.alexlucifer.info/api/user/category/delete/${id}`,{headers:this.header}).then((response)=>{
                    this.categories=response.data;
                    this.loadingStatus=false;
                    this.categoryCount=this.categories.length;
                })
            ...
            searchCategory(page=1){
                if (this.searchKey!='') {
                    this.loadingStatus=true;
                    axios.get(`http://alexmedia.alexlucifer.info/api/user/category/search/${this.searchKey}?page=${page}`,{headers:this.header}).then((response)=>{
                        this.categories=response.data;
                        this.loadingStatus=false;
                        this.categoryCount=this.categories.length;
