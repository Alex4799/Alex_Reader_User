        ...
        methods: {
            getPlaylist () {
                ...
                axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/get/${this.user.id}`,{headers:this.header}).then((response)=>{
                    ...
                    for (let i = 0; i < this.playlistCount; i++) {
                        axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/postCount/${this.platlists[i].id}`,{headers:this.header}).then((response)=>{
            ...
            getSearchPlaylist () {
                if(this.searchKey!='' || this.searchKey!=null){
                    ...
                    axios.post(`https://alexmedia.alexlucifer.com/api/user/playlist/get/search`,data,{headers:this.header}).then((response)=>{
                        this.platlists=response.data;
                        this.playlistCount=this.platlists.length;
                        for (let i = 0; i < this.playlistCount; i++) {
                            axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/postCount/${this.platlists[i].id}`,{headers:this.header}).then((response)=>{
            ...
            deleteItem(id){
                this.loadingStatus=true;
                axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/delete/${id}`,{headers:this.header}).then((response)=>{
                    this.platlists=response.data;
                    this.playlistCount=this.platlists.length;
                    for (let i = 0; i < this.playlistCount; i++) {
                        axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/postCount/${this.platlists[i].id}`,{headers:this.header}).then((response)=>{
