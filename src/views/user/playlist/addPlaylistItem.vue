    ...
    export default {
        ...
        methods: {
            getData () {
                ...
                axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/item/add/getData/${this.$route.params.id}`,{headers:this.header}).then((response)=>{
            ...
            addPost(id){
                this.loadingStatus=true;
                axios.get(`https://alexmedia.alexlucifer.com/api/user/playlist/item/add/${id}/${this.playlist.id}`,{headers:this.header}).then((response)=>{
                    this.setData(response);
                    this.loadingStatus=false;
                });
            },
                ...
                this.posts=response.data.posts;
                    ...
                    for (let i = 0; i < this.posts.length; i++) {
                        if (this.posts[i].image!=null) {
                            this.posts[i].image=`https://alexmedia.alexlucifer.com/storage/${this.posts[i].image}`;
                        }else{
                            this.posts[i].image=`https://alexmedia.alexlucifer.com/image/default.png`;
                        }
